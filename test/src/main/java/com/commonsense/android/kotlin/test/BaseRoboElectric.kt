...
abstract class BaseRoboElectricTest {
     ...
     *
     */
    inline fun <reified T : Activity> createActivity(@StyleRes theme: Int = R.style.Theme_AppCompat): T =
            createActivityController<T>(theme).create().get()

    /**
     * creates an activity controller to test lifecycle events
     */
    inline fun <reified T : Activity> createActivityController(@StyleRes theme: Int = R.style.Theme_AppCompat): ActivityController<T> =
