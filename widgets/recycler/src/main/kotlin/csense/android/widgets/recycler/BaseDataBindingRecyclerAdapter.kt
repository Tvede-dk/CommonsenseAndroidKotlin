...
inline fun RecyclerView.Adapter<*>.notifyItemRangeChanged(range: kotlin.ranges.IntRange) {
...
interface IRenderModelItem<T : Any, Vm : ViewDataBinding> :
     ...
     * @param viewHolder csense.android.widgets.recycler.BaseViewHolderItem<Vm> the view holder containing the view and data
     */
    fun renderFunction(view: Vm, model: T, viewHolder: BaseViewHolderItem<Vm>)

    /**
     * Binds this to the given view holder.
     * @param holder csense.android.widgets.recycler.BaseViewHolderItem<*>
     */
     ...
     * @return csense.android.widgets.recycler.BaseViewHolderItem<Vm> a valid view holder for the given view binding
     */
...
abstract class DataBindingRecyclerAdapter<T>(context: Context) :
     ...
     * @param viewType Int
     * @return csense.android.widgets.recycler.BaseViewHolderItem<*>
     */
     ...
     * @param viewHolderItem csense.android.widgets.recycler.BaseViewHolderItem<*> the view holder to create it from)
     * @return csense.android.widgets.recycler.InflatingFunction<*>? the potential inflater function iff anyone can create the view holders view type.
     * null if none can
     */
    @UiThread
...
//                "We handle everything our self. Please do not call this method.")
    ...
    //TODO make better :)
    //gets the raw index for a given section and row, iff valid (the item is there).
    fun getRawIndexForSection(sectionIndex: Int, rowIndex: Int): Int? {
        val haveSection = dataCollection.getSectionLocation(sectionIndex) ?: return null
        return if (haveSection.inRaw.length < rowIndex) {
            haveSection.inRaw.start + rowIndex
        } else {
            null
        }
 ...
 * @receiver csense.android.widgets.recycler.BaseDataBindingRecyclerAdapter
